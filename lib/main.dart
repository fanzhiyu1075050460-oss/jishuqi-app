...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(debugShowCheckedModeBanner: false, home: HomePage());返回 常量MaterialApp(debugShowCheckedModeBanner：假, home：HomePage());
  }
}
...
class _HomePageState extends State<HomePage> {类_HomePageState扩展State<HomePage> {
  ...
  void doCalculate() {
    ...
    for (final p in controller.text.trim().split(RegExp(r'[,，\s]+'))) {对于 (最终p在控制器文本.trim().split(正则表达式(r'[,，\s]+'))) {
      final n = int.tryParse(p.trim());最终n = int.tryParse(p.trim());
      if (n != null) bases.add(n);如果(n !=空) 基类列表.add(n);
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(主体：内边距（body：内边距（主体：内边距（
        padding: const内边距：const EdgeInsets.all(16),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ElevatedButton.icon( ElevatedButton.icon（ ElevatedButton.icon（ ElevatedButton.icon（
            ...
            icon: const Icon(Icons.copy),图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，图标：常量Icon(Icons.copy)，
            label: const Text('一键复制'),标签：constText('一键复制'),标签：常量文本('一键复制'),标签：常量文本('一键复制'),
          ),
          const SizedBox(height: 12),
          Expanded(child: SingleChildScrollView(child: SelectableText(txt, style: const TextStyle(fontFamily: 'monospace')))),扩展（子组件：SingleChildScrollView（子组件：SelectableText(txt, style：常量TextStyle(字体族：'等宽字体')))),
        ]),
      ),
    );
  }
